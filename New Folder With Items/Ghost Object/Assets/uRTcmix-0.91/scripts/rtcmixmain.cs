...
public class rtcmixmain : MonoBehaviour {
	...
	public void initRTcmix(int objno) {
		...
		AudioSettings.GetDSPBufferSize(out int bufferSize, out int bufferAmount);
		while (DLL.check_context() == 1) System.Threading.Thread.Sleep (2);
		DLL.RTcmix_setparams(AudioSettings.outputSampleRate, 2, bufferSize, 1, 32, objno); // '32' is the nmber of RTcmix busses
    }
